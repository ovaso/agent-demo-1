...
- [运行事实与 Shell/任务入口](06-shell-facts-and-task-flow.md)为用户已明确的新设计：进程级事实仓及内部刷新、默认纯对话、会话内 think 选择、`/resolver`、planner 独立开关和临时协作板登记尚未实现。当前普通输入仍直接创建任务，不应按该设计描述现有 CLI。
- [主/子 Agent 统一执行规格](04-unified-agent-spec.md)及[最小启动与结果接纳](05-agent-startup-and-result.md)目前为设计草案；统一创建结构、角色系统提示词快照、精选材料装配及完整的统一验收尚未实现，现有能力仍以上表为准。
