...
use super::usage;
use agent_core::{
    model::{ModelError, ModelResponse, ModelStreamEvent},
    tool::ToolCall,
};
...
pub(super) fn parse_stream(
        ...
        .lines()
    {
        ...
        {
            stop = super::stop_reason::parse(Some(reason));
        }
    ...
    if !finished {
        return Err(ModelError::new("deepseek 流在结束标记之前中断"));
    }
    if !stop.is_complete() {
        calls.clear();
    }
    let calls = calls
        ...
        .map(|call| {
            if call.id.is_empty() || call.name.is_empty() {
                return Err(ModelError::new("DeepSeek 流式工具调用不完整"));
            }
            Ok(ToolCall::new(
                call.id,
                call.name,
    ...
    let continuation = reasoning.as_ref().map(|text| {
