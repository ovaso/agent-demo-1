...
- DeepSeek：读取 `prompt_tokens`、`completion_tokens`、`prompt_cache_hit_tokens` 和 `completion_tokens_details.reasoning_tokens`；缓存未命中量不当作独立缓存写入用量。
...
`model.response.completed` 将答案的 `emitted_characters` 和思考的 `reasoning_characters` 分别统计；`first_reasoning_ms` 记录首次思考增量延迟，未返回思考时为 null。`model.first_text_delta` 仍指答案文本的首个增量。展示事件不把思考正文、签名或加密块写入这些 trace 字段。
...
各 `agent-vendor-*/src/provider/usage.rs`，终端查看位于 `agent-app/src/trace_map`。
