...
mod tests {
    ...
    #[test]
    fn writes_one_json_event_per_line() {
        let path = std::env::temp_dir().join(format!(
            "rs-agent-trace-test-{}-{}.jsonl",
            std::process::id(),
            SystemTime::now()
                .duration_since(UNIX_EPOCH)
                .unwrap()
        ...
        let mut sink = FileTraceSink::open(&path).unwrap();

        sink.record(TraceEvent::new("test.completed").with_field("count", 1_u64))
            .unwrap();

        let line = std::fs::read_to_string(&path).unwrap();
        let event: Value = serde_json::from_str(line.trim()).unwrap();
        assert_eq!(event["name"], "test.completed");
