...
mod tests {
    use super::*;
    fn update(key: &str, expected_revision: u64) -> BoardUpdate {
        BoardUpdate {
            key: key.into(),
            expected_revision,
    ...
    #[test]
    fn rejects_conflicting_writes_and_keeps_referenced_versions() {
        let mut board = Blackboard::default();
        board.write("a", 1, update("api", 0)).unwrap();
        assert_eq!(
            board.write("b", 1, update("api", 0)).unwrap_err(),
            RuntimeError::Conflict
        );
        board.write("b", 1, update("tests", 0)).unwrap();
        board.write("a", 2, update("api", 1)).unwrap();
        assert_eq!(board.version("api", 1).unwrap().plan_version, 1);
        assert_eq!(board.changes(1, 1)[0].key, "tests");
        let restored: Blackboard =
            serde_json::from_str(&serde_json::to_string(&board).unwrap()).unwrap();
        assert_eq!(restored.version("api", 2).unwrap().author, "a");
        assert_eq!(restored.sequence(), 3);
    }
    #[test]
    fn rejects_oversized_entries_without_advancing_the_cursor() {
        let mut board = Blackboard::default();
        let mut entry = update("large", 0);
        entry.content = "x".repeat(MAX_ENTRY_BYTES + 1);
        assert!(board.write("a", 0, entry).is_err());
        assert_eq!(board.sequence(), 0);
    }
}
