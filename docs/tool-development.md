...
当前宏实现已归入 `agent-core/tool-macro`，包名仍为 `agent-tool-macro`，应用只需使用 `agent-core::tool`。由于 Rust 的过程宏编译要求，内部保留 `proc-macro` 子 crate。2026-09-10 后续调整已按用户要求删除全部自动化测试、夹具与基准程序；本文涉及测试和测量的内容记录此前验证，当前日常检查规则以根目录 `AGENTS.md` 为准。
