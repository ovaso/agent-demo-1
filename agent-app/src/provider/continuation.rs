...
mod tests {
    ...
    use agent_core::{
        context::Message,
        model::{ModelContinuation, ModelResponse},
    };
    use serde_json::json;

    #[test]
    fn continuation_is_bound_to_model_host_and_protocol_without_url_secrets() {
        let key = binding(
            OPENAI,
            "model-a",
            "https://user:secret@example.com:123/v1?token=secret",
        );
        assert!(!key.contains("secret"));
        let mut response = ModelResponse::text("answer").with_continuation(Some(
            ModelContinuation::new(OPENAI, json!({"reasoning_content":"continuation"})),
        ));
        response.bind_continuation(key.clone());
        let (text, calls, continuation) = response.into_reply_parts();
        let message = Message::assistant_reply(text.unwrap(), calls, continuation);
        let message: Message =
            serde_json::from_value(serde_json::to_value(message).unwrap()).unwrap();
        let request = ModelRequest::new(vec![message], &[], &[]);
        assert!(validate(&request, OPENAI, &key).is_ok());
        assert!(
            validate(
                &request,
                OPENAI,
                &binding(OPENAI, "model-b", "https://example.com/v1")
            )
            .is_err()
        );
        assert!(validate(&request, ANTHROPIC, &key).is_err());
    }
}
