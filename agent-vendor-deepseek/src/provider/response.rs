...
pub(super) fn parse_response(response: &Value) -> Result<ModelResponse, ModelError> {
    let stop = super::stop_reason::parse(
        response
    ...
    let message = response
        ...
        .ok_or_else(|| ModelError::new("DeepSeek 响应缺少 choices[0].message"))?;
    let text = message
        .get("content")
        .and_then(Value::as_str)
            ...
            calls
                ...
                .map(|call| {
                    let id = call
                        .get("id")
                        .and_then(Value::as_str)
                        .ok_or_else(|| ModelError::new("DeepSeek 工具调用缺少 id"))?;
                    let function = call
                        .get("function")
                        .ok_or_else(|| ModelError::new("DeepSeek 工具调用缺少 function"))?;
                    let name = function
                        .get("name")
                        .and_then(Value::as_str)
                        .ok_or_else(|| ModelError::new("DeepSeek 工具调用缺少名称"))?;
                    let arguments = function
                        .get("arguments")
                        .and_then(Value::as_str)
                        .ok_or_else(|| ModelError::new("DeepSeek 工具调用缺少参数"))?;
