...
mod tests {
    ...
    fn plan() -> Plan {
        Plan {
            goal: "fix".into(),
            requirements: vec!["tests pass".into()],
            tasks: vec![PlanTask {
                id: "a".into(),
                ...
                action: TaskAction::Agent {
                    prompt: "fix and test".into(),
                },
            }],
        }
    }
    #[test]
    fn validates_dependencies_and_preserves_requirements_across_versions() {
        let mut history = PlanHistory::default();
        history.propose(0, plan()).unwrap();
        assert_eq!(
            history.propose(0, plan()).unwrap_err(),
            RuntimeError::Conflict
        );
        let mut next = plan();
        next.requirements = vec!["different".into()];
        assert!(history.propose(1, next).is_err());
        let mut next = plan();
        next.tasks[0].depends_on.push("a".into());
        assert!(history.propose(1, next).is_err());
        let mut next = plan();
        next.tasks[0].depends_on.push("missing".into());
        assert!(history.propose(1, next).is_err());
        history.propose(1, plan()).unwrap();
        assert_eq!(history.version(1), Some(&plan()));
    }
}
